...
class ExerciseService {
     ...
     */
    static calculateRelevanceScore(exercise, query) {
        ...
        // Some query words appear in name - 100 points per word
            ...
            const matchingWords = queryWords.filter((word) => normalizedName.includes(word));
    ...
    static async getById(exerciseId) {
        try {
            const exercise = await exercise_model_1.ExerciseModel.findById(exerciseId);
            return exercise;
        }
        catch (err) {
            logger_service_1.logger.error(`Failed to get exercise ${exerciseId}`, err);
            throw err;
        }
    }
    /**
    ...
    static async getByExerciseId(exerciseId) {
        try {
            const exercise = await exercise_model_1.ExerciseModel.findOne({ exerciseId });
            return exercise;
        }
        catch (err) {
            logger_service_1.logger.error(`Failed to get exercise by exerciseId ${exerciseId}`, err);
            throw err;
        }
    }
    /**
     ...
     */
    static async getByExerciseIds(exerciseIds) {
        try {
            const exercises = await exercise_model_1.ExerciseModel.find({
    ...
    static async query(filterBy = {}) {
        try {
            const exercises = await exercise_model_1.ExerciseModel.find(filterBy);
            return exercises;
        }
        catch (err) {
            logger_service_1.logger.error('Failed to query exercises', err);
            throw err;
        }
    }
    /**
    ...
    static async searchByName(params) {
        try {
            ...
            logger_service_1.logger.error(`Failed to search exercises by name ${params.query}`, err);
            throw err;
        }
    }
    /**
     ...
     */
    static async getAlternateExercises(exercise) {
        try {
            const backendExercise = await this.getByExerciseId(exercise.exerciseId);
            const normalizedExercise = backendExercise || exercise;
            const exercises = await exercise_model_1.ExerciseModel.find(this._getAlternateExercisesFilter(normalizedExercise))
                .sort({
                popularityScore: -1,
            })
                .select({ createdAt: 0, updatedAt: 0 });
            const sortedExercises = this.getSortedExercises(exercises, normalizedExercise);
            return sortedExercises;
        }
        catch (err) {
            logger_service_1.logger.error(`Failed to get alternate exercises ${exercise}`, err);
            throw err;
        }
    }
    static getSortedExercises(exercises, exercise) {
        return exercises.sort((a, b) => {
            return (this.calculateRelevanceScore(b, exercise.name) -
                this.calculateRelevanceScore(a, exercise.name));
        });
    }
    static _getAlternateExercisesFilter(exercise) {
    ...
    static getExerciseNormalizedName(name) {
        const wordsToRemove = [
            'lever',
            'machine',
            'dumbbell',
     ...
     */
    static async searchByMuscleGroups(muscleGroups) {
        try {
            const exercises = await exercise_model_1.ExerciseModel.find({
            ...
            logger_service_1.logger.error(`Failed to search exercises by muscle groups ${muscleGroups}`, err);
            throw err;
        }
    }
    /**
     ...
     */
    static async searchByEquipment(equipment) {
        try {
            const exercises = await exercise_model_1.ExerciseModel.find({
                $or: [
            ...
            logger_service_1.logger.error(`Failed to search exercises by equipment ${equipment}`, err);
            throw err;
        }
    }
    /**
    ...
    static async add(exercise) {
        try {
            ...
            if (exercise.exerciseId) {
                const existing = await exercise_model_1.ExerciseModel.findOne({
                    exerciseId: exercise.exerciseId,
                });
                if (existing) {
                    throw new Error(`Exercise with exerciseId ${exercise.exerciseId} already exists`);
                }
            }
            const addedExercise = await exercise_model_1.ExerciseModel.create(exercise);
     ...
     */
    static async addMany(exercises) {
        try {
            ...
            if (exerciseIds.length > 0) {
                const existing = await exercise_model_1.ExerciseModel.find({
                    exerciseId: { $in: exerciseIds },
                });
                const existingIds = new Set(existing.map((ex) => ex.exerciseId).filter(Boolean));
                const newExercises = exercises.filter((ex) => !existingIds.has(ex.exerciseId));
                if (newExercises.length === 0) {
                    return existing;
                }
                const savedExercises = await exercise_model_1.ExerciseModel.insertMany(newExercises, {
                    ordered: false,
                }).catch((err) => {
                    // If there are duplicate key errors, fetch existing ones
                    if (err.code === 11000 || err.name === 'MongoServerError') {
                        logger_service_1.logger.info(`Some exercises already exist, fetching existing ones`);
            ...
            const savedExercises = await exercise_model_1.ExerciseModel.insertMany(exercises, {
                ordered: false,
            });
     ...
     * Update an exercise
     */
    static async update(exerciseId, exerciseToUpdate) {
        try {
            const exercise = await exercise_model_1.ExerciseModel.findByIdAndUpdate(exerciseId, exerciseToUpdate, {
                new: true,
            });
            return exercise;
        }
        catch (err) {
            logger_service_1.logger.error(`Failed to update exercise ${exerciseId}`, err);
            throw err;
        }
    }
    /**
     ...
     */
    static async updateByExerciseId(exerciseId, exerciseToUpdate) {
        try {
            const exercise = await exercise_model_1.ExerciseModel.findOneAndUpdate({ exerciseId }, exerciseToUpdate, {
                new: true,
            });
            return exercise;
        }
        catch (err) {
            logger_service_1.logger.error(`Failed to update exercise by exerciseId ${exerciseId}`, err);
            throw err;
        }
    }
    /**
     * Remove an exercise
     */
    static async remove(exerciseId) {
        try {
            await exercise_model_1.ExerciseModel.findByIdAndDelete(exerciseId);
        }
        catch (err) {
            logger_service_1.logger.error(`Failed to remove exercise ${exerciseId}`, err);
            throw err;
        }
    }
    /**
     ...
     */
    static async removeByExerciseId(exerciseId) {
        try {
            await exercise_model_1.ExerciseModel.findOneAndDelete({ exerciseId });
        }
        catch (err) {
            logger_service_1.logger.error(`Failed to remove exercise by exerciseId ${exerciseId}`, err);
            throw err;
        }
    }
}
