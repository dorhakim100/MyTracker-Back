...
class MenuService {
    ...
    static async select(menu) {
        try {
            ...
            await menu_model_1.Menu.updateMany({ userId }, { $set: { isSelected: false } });
            const savedMenu = await menu_model_1.Menu.findByIdAndUpdate(menuId, { isSelected: true }, { new: true }).populate('menuLogs');
            return savedMenu;
        }
        catch (err) {
