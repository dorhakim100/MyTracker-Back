...
class MenuController {
  ...
  static async getMenusByUser(req, res) {
    try {
      const menus = await menu_service_1.MenuService.getByUserId(
        req.params.userId
      )
      res.json(menus)
    } catch (err) {
  ...
  static async selectMenu(req, res) {
    try {
      const { menu } = req.body
      const savedMenu = await menu_service_1.MenuService.select(menu)
      res.json(savedMenu)
    } catch (err) {
      logger_service_1.logger.error('Failed to select menu', err)
      res.status(500).send({ err: 'Failed to select menu' })
    }
  }
  static async updateMenu(req, res) {
    try {
      const menu = await menu_service_1.MenuService.update(
        req.params.id,
        req.body
      )
      res.json(menu)
