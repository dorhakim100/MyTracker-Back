...
class DayService {
    static async upsertFromLoggedToday(params) {
        try {
            ...
            const logIdsAsStrings = (logs || [])
                ...
                .filter(Boolean);
            const day = await day_model_1.Day.findOneAndUpdate({ userId, date }, { $set: { logs: logIdsAsStrings, calories } }, { new: true, upsert: true });
            return day;
        }
        catch (err) {
            logger_service_1.logger.error('DayService.upsertFromLoggedToday failed', err);
            throw err;
        }
    }
    static async getById(dayId) {
        try {
            ...
            logger_service_1.logger.error(`DayService.getById failed for ${dayId}`, err);
            throw err;
        }
    }
    static async getByUserAndDate(userId, date) {
        try {
            ...
            const isExistingDay = await day_model_1.Day.findOne({ userId, date: dateFromISO });
            if (!isExistingDay) {
            ...
            logger_service_1.logger.error(`DayService.getByUserAndDate failed for user ${userId} date ${date}`, err);
            throw err;
        }
    }
    static async listByUser(userId, limit = 30) {
        try {
            ...
            logger_service_1.logger.error(`DayService.listByUser failed for ${userId}`, err);
            throw err;
        }
    }
    ...
    static async update(day, userId) {
        try {
            ...
            const updatedDay = await day_model_1.Day.findByIdAndUpdate(dayId, { ...dayToUpdate }, { new: true });
            return updatedDay;
        }
        catch (err) {
