...
class GoogleOAuthService {
  static getAuthorizationUrl(intent, returnTo) {
  ...
  static async linkOrCreateUser(profile, encryptedRefreshToken) {
    ...
    const existingByEmail = await user_model_1.User.findOne({
      email: profile.email,
    })
    const action = (0, google_oauth_logic_1.resolveGoogleLinkOrCreateAction)(
      existingByGoogleId
        ? {
    ...
    switch (action.type) {
      case 'login':
        ...
        break
      case 'link':
        user = await user_model_1.User.findByIdAndUpdate(
          action.userId,
          {
            googleId: profile.googleId,
        ...
        break
      case 'create': {
        const defaultLoggedToday =
          day_service_1.DayService.getDefaultLoggedToday()
    ...
    const aggregatedUser = await user_service_1.UserService.getById(
      String(user._id)
    )
