...
class SetService {
    ...
    static async query(filterBy = {}) {
        try {
            logger_service_1.logger.info('filterBy', filterBy);
            ...
            const criteria = { ...rest, isDone: true };
            if (from)
                criteria.createdAt = { $gte: from };
            logger_service_1.logger.info('criteria', criteria);
            let query = set_model_1.Set.find(criteria);
