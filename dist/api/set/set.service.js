...
class SetService {
  ...
  static async query(filterBy = {}) {
    try {
      const sets = await set_model_1.Set.find({
        ...filterBy,
        isDone: true,
      }).sort({
        setNumber: 1,
  ...
  static async getBySessionId(sessionId) {
    try {
      ...
      logger_service_1.logger.error(
        `Failed to get sets by session id ${sessionId}`,
        err
      )
      throw err
    }
  ...
  static async removeBySessionId(sessionId) {
    try {
      ...
      logger_service_1.logger.error(
        `Failed to remove sets by session id ${sessionId}`,
        err
      )
      throw err
    }
  ...
  static async saveBySessionIdAndExerciseIdAndSetIndex(
    ...
    try {
      ...
      const savedSet = await set_model_1.Set.findOneAndUpdate(
  ...
  static async removeBySessionIdAndExerciseIdAndSetIndex(
    ...
    try {
      await set_model_1.Set.deleteOne({
        sessionId,
        exerciseId,
        setNumber: setIndex + 1,
      })
